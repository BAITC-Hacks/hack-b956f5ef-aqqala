...
- **Бюджет хакатона ($50 OpenAI + $50 NVIDIA).** Число запросов и токенов за каждый запуск показывается в интерфейсе и возвращается агентом (`usage`), так что расход можно проконтролировать.
...
2. На [share.streamlit.io](https://share.streamlit.io) → **Create app**: репозиторий, ветка `main`, файл `app.py`, в Advanced settings — Python 3.12. Зависимости берутся из `requirements.txt`.
...
- **LLM-режим проверен автотестом на сценарном (подменённом) клиенте и на откате без ключа.** Прогон с реальным ключом OpenAI или NVIDIA в этой версии не выполнялся, а выбранные модели по умолчанию нужно подтвердить на доступном аккаунте.
